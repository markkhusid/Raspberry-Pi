...
int main(int argc, char* argv[]) {
    ...
    if (strcmp(argv[1], "setup") == 0) {
        ...
        writeGPIO("/sys/class/gpio/" "export", "4");
        usleep(100000);
        writeGPIO("/sys/class/gpio/gpio4/" "direction", "out");
    }
    else if (strcmp(argv[1], "close") == 0) {
        printf("Closing the LED on the GPIO\n");
        writeGPIO("/sys/class/gpio/" "unexport", "4");
    }
    else if (strcmp(argv[1], "on") == 0) {
        printf("Turning the LED on\n");
        writeGPIO("/sys/class/gpio/gpio4/" "value", "1");
    }
    else if (strcmp(argv[1], "off") == 0) {
        printf("Turning the LED off\n");
        writeGPIO("/sys/class/gpio/gpio4/" "value", "0");
    }
    else if (strcmp(argv[1], "status") == 0) {
        FILE *fp;
        char line[80], fullFilename[100];
        sprintf(fullFilename, "/sys/class/gpio/gpio4/" "/value");
